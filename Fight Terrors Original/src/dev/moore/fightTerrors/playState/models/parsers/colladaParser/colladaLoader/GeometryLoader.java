...
import dev.moore.fightTerrors.playState.models.parsers.colladaParser.dataStructures.AnimVertex;
import dev.moore.fightTerrors.playState.models.parsers.colladaParser.dataStructures.VertexSkinData;
...
public class GeometryLoader {
	...
	List<AnimVertex> vertices = new ArrayList<AnimVertex>();
	...
	private void readPositions() {
		...
		for (int i = 0; i < count / 3; i++) {
			...
			position = Matrix4f.transform(CORRECTION, position, null);
			vertices.add(new AnimVertex(vertices.size(), new Vector3f(position.x, position.y, position.z),
	...
	private void readNormals() {
		...
		for (int i = 0; i < count / 3; i++) {
			...
			norm = Matrix4f.transform(CORRECTION, norm, null);
	...
	private AnimVertex processVertex(int posIndex, int normIndex, int texIndex) {
		AnimVertex currentVertex = vertices.get(posIndex);
	...
	private float convertDataToArrays() {
		...
		for (int i = 0; i < vertices.size(); i++) {
			AnimVertex currentVertex = vertices.get(i);
	...
	private AnimVertex dealWithAlreadyProcessedVertex(AnimVertex previousVertex, int newTextureIndex, int newNormalIndex) {
		if (previousVertex.hasSameTextureAndNormal(newTextureIndex, newNormalIndex)) {
			indices.add(previousVertex.getIndex());
			return previousVertex;
		} else {
			AnimVertex anotherVertex = previousVertex.getDuplicateVertex();
			if (anotherVertex != null) {
				return dealWithAlreadyProcessedVertex(anotherVertex, newTextureIndex, newNormalIndex);
			} else {
				AnimVertex duplicateVertex = new AnimVertex(vertices.size(), previousVertex.getPosition(),
